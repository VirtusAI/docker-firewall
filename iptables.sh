...
iptables-save -t filter | sed "/$prefix-/d;/^COMMIT$/d;/^#/d;/FILTERS/d"

echo -N FILTERS
echo -A INPUT -j FILTERS
echo -A DOCKER-USER -i $IF -j FILTERS
...
# Insert rule into FILTERS
for proto in ${protos//,/$'\n'}; do
  for port in ${ports//,/$'\n'}; do
    echo -A FILTERS -p ${proto} -m ${proto} --dport ${port} -j ${chain}
  done
done
